 ...
 *  Simulator
 *
 *  Created by Stefan Jorgensen on 10/12/13.
...
bool Simulator::get_meas(int j, int k, float* r, coord u)
{
	if(j != k)
	{
		...
		//Pass noisy distance
		r[0] = r_j + inject_noise(STDEVS[SDEV_R]); //add measurement noise

		//calculate direction vector
		u_j[X] = (NODES[j]->get_w_0(X) - NODES[k]->get_w_0(X));
		u_j[Y] = (NODES[j]->get_w_0(Y) - NODES[k]->get_w_0(Y));
		float n_theta = inject_noise(STDEVS[SDEV_U]);
		...
		u[X] = NODES[j]->get_w_0(X) + inject_noise(STDEVS[SDEV_G]);
		u[Y] = NODES[j]->get_w_0(Y) + inject_noise(STDEVS[SDEV_G]);
		return true;
	}
...
float Simulator::inject_noise(float var)
{
	//use central limit threorem to generate gaussian noise
	float n = 0;
	for(int k = 0; k < 36; k++)
		n+= rand()%100001 - 50000; // 5 sig figs, zero mean
	n /= (6*100000/sqrt(12)); //CLT
	
	//n is now unit variance and zero mean
	//to add variance, multiply by sqrt var:
	n = n*sqrt(var);
